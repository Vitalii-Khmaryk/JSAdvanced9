...
describe('CategoryService', () => {
  ...
  it('can test HttpClient.post ', () => {
    ...
    service.createCategory(data).subscribe((response) => expect(response).toBeNull());
    const req = backEnd.expectOne(url + `/categories`);
  ...
  it('can test HttpClient.delete ', () => {
    const id = 1;
    service.deleteCategory(id).subscribe((response) => expect(response).toBeNull());
  ...
  it('can test HttpClient.patch ', () => {
    const data = { name: 'category', path: 'qqq', imagePath: 'qqq' };
    const id = 1;
    service.updateCategory(data, id).subscribe((response) => {
